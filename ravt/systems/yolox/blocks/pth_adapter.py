...
def yolox_pth_adapter(yolox_pth_dict: Dict, remove_head: bool = True) -> Dict:

    new_ckpt = {}
    for k, v in yolox_pth_dict['model'].items():
        if remove_head and 'head' in k:
            continue
...
def get_pretrained_pth_adapter(filename: str) -> Callable[[Path], Dict]:
    def pth_adapter(pretrained_dir: Path) -> Dict:
        file = pretrained_dir.joinpath(filename)
        if file.exists():
            pth = torch.load(str(file), map_location='cpu')
